...
public class Lottery extends JFrame
{
    ...
    JCheckBox cbTheSameQuickSelect;
        ...
        btnSelfSelect.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                } else
                {
                    ...
                    {
                        dListModel.addElement(SelectNumToString(hsMySelectBtn)); // 依照組數將號碼加入JList
                    }
            ...
            {   
              //勾選同號快選時，加入清單的號碼一樣
                if(cbTheSameQuickSelect.isSelected()) {
                    ...
                    for (int i = 0; i < (Integer) spinnerSets.getValue(); i++)
                    {
                        alSelectNum.add(hsQuickSelectBtn.iterator()); // 存入ArrayList
                        dListModel.addElement(SelectNumToString(hsQuickSelectBtn)); //轉成字串並加入選號清單Jlist                        
                    }
                    SelectBtnClear(hsQuickSelectBtn); // 清除快選的號碼
                    SelectBtnClear(hsMySelectBtn); // 清除自選的號碼
                }else { //未勾選同號快選時，加入清單的號碼每次均為隨機
                    // 依照組數將號碼加入JList
                    for (int i = 0; i < (Integer) spinnerSets.getValue(); i++)
                    {
                        hsQuickSelectBtn.addAll(hsMySelectBtn); //先加入自選的號碼                        
